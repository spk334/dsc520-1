...
## Use `setwd()` if needed
setwd("C:/Users/kkooken/Documents/EDU/520/R/dsc520-1/data/tidynomicon")

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv("person.csv", stringsAsFactors = TRUE)
str(person_df1)
...
person_df2 <- read.csv("person.csv", stringsAsFactors = FALSE)
str(person_df2)
...
voter_turnout_df1 <- read_excel('../../data/G04ResultsDetail2004-11-02.xls',s=1, sheet="Voter Turnout")
...
voter_turnout_df2 <- read_excel('../../data/G04ResultsDetail2004-11-02.xls',s=2, 
