...
## assuming first entry is Sunday night's sleep 
week1_sleep[3]
...
factor_recommendations_vector <- factor(
  ...
  ordered = is.ordered(factor_genre_vector),
...
## Using the built-in mtcars dataset, view the last few rows using the `tail()` functiont
tail(mtcars)
